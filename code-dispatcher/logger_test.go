...
func TestLoggerCreatesFileWithPID(t *testing.T) {
	...
	expectedPath := filepath.Join(tempDir, fmt.Sprintf("code-dispatcher-%d.log", os.Getpid()))
...
func TestLoggerCleanupOldLogsRemovesOrphans(t *testing.T) {
	...
	orphan1 := createTempLog(t, tempDir, "code-dispatcher-111.log")
	orphan2 := createTempLog(t, tempDir, "code-dispatcher-222-suffix.log")
	running1 := createTempLog(t, tempDir, "code-dispatcher-333.log")
	running2 := createTempLog(t, tempDir, "code-dispatcher-444-extra-info.log")
...
func TestLoggerCleanupOldLogsHandlesInvalidNamesAndErrors(t *testing.T) {
	...
	invalid := []string{
		"code-dispatcher-.log",
		"code-dispatcher.log",
		"code-dispatcher-foo-bar.txt",
		"not-a-codex.log",
	}
	for _, name := range invalid {
		createTempLog(t, tempDir, name)
	}
	target := createTempLog(t, tempDir, "code-dispatcher-555-extra.log")
...
func TestLoggerCleanupOldLogsHandlesTempDirPermissionErrors(t *testing.T) {
	...
	paths := []string{
		createTempLog(t, tempDir, "code-dispatcher-6100.log"),
		createTempLog(t, tempDir, "code-dispatcher-6101.log"),
	}
...
func TestLoggerCleanupOldLogsHandlesPermissionDeniedFile(t *testing.T) {
	...
	protected := createTempLog(t, tempDir, "code-dispatcher-6200.log")
	deletable := createTempLog(t, tempDir, "code-dispatcher-6201.log")
...
func TestLoggerCleanupOldLogsPerformanceBound(t *testing.T) {
	...
	for i := 0; i < fileCount; i++ {
		name := fmt.Sprintf("code-dispatcher-%d.log", 10000+i)
...
func TestLoggerCleanupOldLogsKeepsCurrentProcessLog(t *testing.T) {
	...
	currentLog := createTempLog(t, tempDir, fmt.Sprintf("code-dispatcher-%d.log", currentPID))
...
func TestLoggerIsUnsafeFileSecurityChecks(t *testing.T) {
	...
	t.Run("symlink", func(t *testing.T) {
		...
		unsafe, reason := isUnsafeFile(filepath.Join(absTempDir, "code-dispatcher-1.log"), tempDir)
	...
	t.Run("outside temp dir", func(t *testing.T) {
		...
		otherDir := t.TempDir()
		stubEvalSymlinks(t, func(string) (string, error) {
			return filepath.Join(otherDir, "code-dispatcher-9.log"), nil
		})
		unsafe, reason := isUnsafeFile(filepath.Join(otherDir, "code-dispatcher-9.log"), tempDir)
		if !unsafe || reason != "file is outside tempDir" {
...
func TestLoggerParsePIDFromLog(t *testing.T) {
	...
	tests := []struct {
		...
		{"code-dispatcher-123.log", 123, true},
		{"code-dispatcher-999-extra.log", 999, true},
		{"code-dispatcher-.log", 0, false},
		{"invalid-name.log", 0, false},
		{"code-dispatcher--5.log", 0, false},
		{"code-dispatcher-0.log", 0, false},
		{fmt.Sprintf("code-dispatcher-%s.log", hugePID), 0, false},
	}
